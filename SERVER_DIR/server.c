...
#define MAXLEN 1400				// maximum data length
...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		else
		{
			...
			memcpy(total_frag_str, buf, 100);
			
			char frag_no_str[100];
			memcpy(frag_no_str, &(buf[100]), 100);
			
			char size_str[100];
			memcpy(size_str, &(buf[200]), 100);

			char newFileName[100];
			memcpy(newFileName, &(buf[300]), 100);

			char newFileName1[100];
			memcpy(newFileName1, newFileName, strlen(newFileName)-1);

			// printf("newFileName1 = %s, length=%d\n", newFileName1, strlen(newFileName1));

			FILE *fp;

			fp = fopen(newFileName1 , "a" );
			fwrite(&(buf[400]) , 1 , 1000 , fp); 
